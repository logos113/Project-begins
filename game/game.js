...
const 설정 = {
  ...
  */
  ...
  /*
    ...
    그만큼 모아야 하는 아이템도 늘어납니다.

    스테이지 1 → 체력 3, 아이템 4개 필요
    스테이지 2 → 체력 4, 아이템 5개 필요
    스테이지 3 → 체력 5, 아이템 6개 필요   ...
  */
  ...
  /*
    체력보다 아이템을 몇 개 더 모아야 하나.
    딱 맞게만 두면 한 발만 빗나가도 이길 수 없습니다. 한 발의 여유를 줍니다.
  */
  보스_여유미사일: 1,
...
// 체력보다 한 발 더 모아야 보스가 나옵니다 (빗나갈 여유)
function 이번보스_필요아이템() {
  return 이번보스_체력() + 설정.보스_여유미사일;
}
...
function 한장면_그리기(지금) {
  ...
  if (!보스 && 점수 >= 다음보스점수) {
    /*
      아이템이 모자라면 이번 보스는 조용히 지나갑니다. 다음 기회에 다시 옵니다.

      예전에는 "아이템이 모자라 보스가 지나갔습니다" 라고 알렸는데,
      아이템을 안 모으고 달리기만 하면 300점마다 계속 뜹니다.
      일어나지 않은 일을 굳이 알릴 이유가 없어서 뺐습니다.
    */
    if (가진미사일 >= 이번보스_필요아이템()) 보스_부르기();
    다음보스점수 += 설정.보스_나오는점수;
  }
