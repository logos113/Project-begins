...
*/
function 학술지_순서_섞기(학술지들, 시드) {
  return [...학술지들]
    .map((이름) => {
      // 이름을 숫자로 바꾼 뒤, 날짜(시드)를 곱해 뒤섞습니다.
      // 나머지 연산과 함께 쓰면 날마다 전혀 다른 순서가 만들어집니다.
      let 점수 = 글자를_숫자로(이름);
      점수 = (점수 * ((시드 % 1000003) + 1)) % 1000000007;
      점수 = (점수 * 48271) % 1000000007;      // 한 번 더 섞어 고르게 퍼뜨립니다
      return { 이름, 점수 };
    })
    .sort((가, 나) => 가.점수 - 나.점수)      // 점수가 작은 것부터
    .map((항목) => 항목.이름);
}
...
function 오늘의_논문_고르기(요약목록) {
  ...
  const 섞인학술지 = 학술지_순서_섞기([...학술지별.keys()], 시드);
