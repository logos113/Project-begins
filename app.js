...
// (2단계) 논문 번호로 제목·저자·초록 등 상세 정보를 받아옵니다
...
async function 논문_불러오기() {
  ...
  try {
    ...
    const 오늘의번호 = 오늘의_논문_고르기(번호목록);
    const 논문들 = await 논문상세_받아오기(오늘의번호);
    ...
    상태숨김();
    오늘날짜_자리.textContent =
      `${오늘_날짜글자()} 기준 · 검색된 ${번호목록.length}편 중 ${논문들.length}편 선정`;
