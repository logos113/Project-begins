/*
  ...
  ※ 영역(scope) 주의 — 이 파일은 psychiatry/ 폴더 안에 있습니다.
...
*/
const 보관함_이름 = "psychiatry-v1";

/*
  미리 받아 둘 파일 목록입니다.

  ※ 이 앱은 논문을 PubMed 에서 받아오므로 '완전한 오프라인' 은 될 수 없습니다.
     인터넷이 없으면 화면은 평소처럼 뜨고, 논문 자리에
     "논문을 불러오지 못했습니다" 안내가 나옵니다.
     빈 화면이 뜨는 것보다는 훨씬 낫고, 저장해 둔 논문은 그대로 볼 수 있습니다.

  ※ 주소에 ?v=16 같은 꼬리표를 붙이지 않았습니다.
...
self.addEventListener("activate", (사건) => {
  사건.waitUntil((async () => {
    ...
    await Promise.all(이름들
      .filter((이름) => 이름.startsWith("psychiatry-") && 이름 !== 보관함_이름)
