...
/*
  이번에 페이지를 연 뒤로 화면에 떴던 논문 번호들입니다.

  '다른 논문 보기'를 눌렀을 때 방금 본 것이 또 나오지 않게 하려고 둡니다.
  페이지를 새로 열면 이 상자는 비워지므로, 같은 날 다시 열면
  오늘의 논문 3편이 그대로 다시 나옵니다.
*/
let 이번에_본_번호 = new Set();
...
/* ==========================================================
   [2-3] 이미 본 논문 기억하기
   ------------------------------------------------------------
   매일 열었을 때 어제 본 논문이 또 나오면 볼 이유가 줄어듭니다.
   그래서 화면에 띄운 논문 번호를 적어두고, 다음부터는 후보에서 뺍니다.

   저장 방식은 북마크와 같습니다 — 이 기기의 브라우저에만 남고 서버로 가지 않습니다.
   저장되는 모양:  { "40123456": "2026-09-12", ... }   (논문 번호 -> 처음 본 날)

   ※ 날짜를 함께 적어두는 데는 두 가지 이유가 있습니다.
      (1) 오늘 본 것은 빼지 않아야 합니다. 그래야 같은 날 다시 열었을 때
          '오늘의 논문' 3편이 그대로 나옵니다.
      (2) 오래된 기록은 버려야 저장 공간이 무한정 늘지 않습니다.
   ========================================================== */

const 본논문_보관함_이름 = "psychiatry-digest-seen";

// 이 기간이 지난 기록은 버립니다. 검색 기간이 최대 1년이라 그보다 오래되면 어차피 안 나옵니다.
const 본논문_보관_일수 = 400;

// 기록이 아무리 쌓여도 이 개수를 넘지 않게 합니다 (저장 공간 보호)
const 본논문_최대개수 = 2000;

function 본논문_불러오기() {
  try {
    const 글 = localStorage.getItem(본논문_보관함_이름);
    const 값 = 글 ? JSON.parse(글) : {};
    // 예전 형식이거나 망가진 값이 들어 있어도 페이지가 멈추지 않게 합니다
...
function 본논문_저장하기(기록) {
  try {
    localStorage.setItem(본논문_보관함_이름, JSON.stringify(기록));
    return true;
  } catch {
    return false;   // 사생활 보호 모드 등으로 저장이 막혀도 그냥 넘어갑니다
  }
}

// "2026-09-12" 가 오늘로부터 며칠 이상 지난 날인지 봅니다
function 오래된_날짜인가(날짜글자, 일수) {
  const [년, 월, 일] = String(날짜글자).split("-").map(Number);
  if (!년 || !월 || !일) return true;            // 이상한 값이면 버립니다
  const 그날 = new Date(년, 월 - 1, 일);
  const 지난날수 = (Date.now() - 그날.getTime()) / (1000 * 60 * 60 * 24);
  return 지난날수 > 일수;
}

// 오래된 기록과 넘치는 기록을 걸러낸 새 기록을 돌려줍니다
function 본논문_정리(기록) {
  let 칸들 = Object.entries(기록)
    .filter(([, 날짜]) => !오래된_날짜인가(날짜, 본논문_보관_일수));

  // 그래도 많으면 최근에 본 것부터 남깁니다
  if (칸들.length > 본논문_최대개수) {
    칸들.sort((가, 나) => (가[1] < 나[1] ? 1 : -1));   // 날짜 내림차순
    칸들 = 칸들.slice(0, 본논문_최대개수);
  }
  return Object.fromEntries(칸들);
}

// 화면에 띄운 논문들을 '본 것'으로 적어둡니다
function 본논문_기록하기(번호들) {
  const 기록 = 본논문_불러오기();
  const 오늘 = 오늘_날짜글자();
  for (const pmid of 번호들) {
    if (!기록[pmid]) 기록[pmid] = 오늘;    // 처음 본 날을 그대로 둡니다
    이번에_본_번호.add(pmid);
  }
  본논문_저장하기(본논문_정리(기록));
}
...
async function 논문_불러오기() {
  ...
  try {
    ...
    if (걸러낸것.모자랐나) {
      상태표시(
        "이미 본 논문을 빼면 3편이 안 되어, 이번에는 빼지 않고 보여드립니다. " +
        "'발표 시기'를 최근 90일이나 1년으로 늘리면 새 논문이 더 나옵니다."
      );
    } else {
      상태숨김();
    }
...
if (본논문_선택창) {
  본논문_선택창.addEventListener("change", () => {
    넘긴_횟수 = 0;
    // '다시 봐도 괜찮음'으로 바꾸면 이번에 본 것도 다시 후보에 넣습니다
    if (본논문_선택창.value !== "exclude") 이번에_본_번호 = new Set();
    논문_불러오기();
  });
}
...
/*
  '본 기록 지우기' 단추.
  이 단추는 날짜 줄을 다시 그릴 때마다 새로 만들어지므로,
  단추에 직접 연결하지 않고 날짜 줄 전체에 한 번만 연결해 둡니다.
*/
if (오늘날짜_자리) {
  오늘날짜_자리.addEventListener("click", (사건) => {
    if (!사건.target.closest("#forgetSeen")) return;
    const 기록수 = Object.keys(본논문_불러오기()).length;
    if (!confirm(`이미 본 논문 ${기록수}편의 기록을 지울까요?\n\n` +
                 `지우면 예전에 본 논문도 다시 나올 수 있습니다.`)) return;
    본논문_저장하기({});
    이번에_본_번호 = new Set();
    넘긴_횟수 = 0;
    논문_불러오기();
