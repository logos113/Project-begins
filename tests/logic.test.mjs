...
// ---- 4. 매일 3편 선정 로직 검사 ----
const 번호들 = Array.from({ length: 60 }, (_, i) => "id" + i);
const 첫번째 = T.오늘의_논문_고르기(번호들);
const 두번째 = T.오늘의_논문_고르기(번호들);
검사("정확히 3편을 고르는가", 첫번째.length === 3, 첫번째);
검사("같은 날 다시 열면 같은 3편이 나오는가", JSON.stringify(첫번째) === JSON.stringify(두번째), [첫번째, 두번째]);
검사("고른 3편에 중복이 없는가", new Set(첫번째).size === 3, 첫번째);
// 논문이 2편밖에 없는 상황에서도 무한반복/오류 없이 처리되는가
const 부족 = T.오늘의_논문_고르기(["a", "b"]);
검사("논문이 2편뿐이면 중복 없이 2편만 보여주는가", 부족.length === 2 && new Set(부족).size === 2, 부족);
