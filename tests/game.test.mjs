...
await 새판();
await page.evaluate(() => {
  설정.보스_공격간격 = 100000; 설정.보스_최소공격간격 = 100000;        // 이 검사에서는 쏘지 않게 해서 죽지 않게 합니다
...
// 이기지 못하고 물러간 것이므로 스테이지는 그대로여야 합니다
검사("보스가 물러가면 스테이지는 그대로인가", 물러간뒤.스테이지 === 1, 물러간뒤.스테이지);
...
검사("보스가 지나가면 스테이지는 그대로인가", 지나감.스테이지 === 1, 지나감.스테이지);
...
await page.evaluate(() => {
  ...
  */
  설정.보스_나오는점수 = 150;
  설정.보스_여유미사일 = 0;      // 딱 체력만큼만 모으면 보스가 나오게
  다음보스점수 = 150;
   ...
   ========================================================== */
{
  ...
  검사("체력이 늘면 모아야 할 아이템도 는다", 이.필요 > 일.필요, [일.필요, 이.필요]);
  검사("모아야 할 아이템이 체력보다 많다 (빗나갈 여유)",
    표.every((칸) => 칸.필요 > 칸.체력), 표.map((칸) => [칸.체력, 칸.필요]));
