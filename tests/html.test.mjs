...
/* ---- 3. 화면 맨 앞에 엉뚱한 글자가 새어 나오지 않는가 ----
   주석 안에 주석을 넣으면 가장 먼저 나오는 닫는 표시에서 끝나버려,
   그 뒤의 설명글이 화면 맨 위에 그대로 노출됩니다. 실제로 겪은 일입니다.

   예전에는 "본문이 'Psychiatry Daily Digest' 로 시작하는가" 로 확인했는데,
   머리말 위에 정상적인 줄(첫 화면으로 돌아가는 링크)을 하나 더하자
   아무 문제가 없는데도 검사가 깨졌습니다.
   보려던 것은 '첫 글자가 무엇인가' 가 아니라 '엉뚱한 글자가 새지 않았는가' 이므로,
   그쪽을 직접 확인하도록 바꿨습니다. */

// (1) 화면에 보이는 첫 글자가 머리말 안에서 나온 것인가.
//     주석이 새면 머리말 바깥(body 바로 밑)에 글자가 생깁니다.
const 글자찾기 = (뿌리) => {
  const 훑기 = 문서.createTreeWalker(뿌리, 4 /* 글자 마디만 */);
  let 마디;
  while ((마디 = 훑기.nextNode())) {
    if (마디.textContent.trim()) return 마디;
  }
  return null;
};
const 첫마디 = 글자찾기(문서.body);
const 머리말 = 문서.querySelector("header.site-header");
검사("화면의 첫 글자가 머리말 안에서 나오는가 (주석이 새지 않았는가)",
  !!첫마디 && !!머리말 && 머리말.contains(첫마디),
  첫마디 && 첫마디.textContent.trim().slice(0, 60));

// (2) 주석 부스러기가 본문에 섞여 있지 않은가
const 본문글 = 문서.body.textContent;
const 부스러기 = ["-->", "<!--", "============", "※ 주의"]
  .filter((조각) => 본문글.includes(조각));
검사("본문에 주석 부스러기가 없는가", 부스러기.length === 0, 부스러기);
...
/*
  브라우저는 한글을 '아무 글자 사이에서나 끊어도 되는 글'로 취급합니다.
  그대로 두면 "그대로 보여줍니다." 가 "…보여줍니" / "다." 로 나뉘어
  마지막 줄에 글자 하나만 덩그러니 남습니다.
*/
검사("한국어를 단어 중간에서 끊지 않는 규칙이 있는가", /word-break:\s*keep-all/.test(css));
검사("아주 긴 낱말은 예외로 끊도록 해두었는가", /overflow-wrap:\s*anywhere/.test(css));
